...
For computational reasons, CNVxplorer only stores in memory the top 5% of the background distribution values generated in the OSS-PV approach (see above), which allow us to estimate significant p-values < 0.05. Thus, when clinical similarity scores were equal or above the 0.05 threshold, are consider not statistically significant and are simply displayed with a slash '-' symbol.
