...
')),
      ...
      tablerTabItems(
          ...
          # fluidRow(
            ...
            column(width = 6,
                            ...
                            tablerCard(width = 12,
                                       ...
                                       status = 'primary',
                                       statusSide = 'left',
                                       title = tagList(shiny::icon("database"), "CNV databases"),
                                       fluidRow(
          ...
          fluidRow(
                    ...
                    column(width = 6,
                           tablerCard(width = 12,
                                      ...
                                      status = 'success',
                                      statusSide = 'left',
                                      title = tagList(shiny::icon("book"), "Phenotypic analysis"),
                                      fluidRow(
                                        column(width = 6,
            ...
            column(width = 6,
                   ...
                   tablerCard(width = 12,
                              ...
                              status = 'success',
                              statusSide = 'left',
                              title = tagList(shiny::icon("book"), "Regulatory regions"),
          ...
          fluidRow(
            ...
            column(width = 12,
                   ...
                   tablerCard(width = 12,
                              ...
                              status = 'primary',
                              statusSide = 'left',
                              title = tagList(shiny::icon("book"), "Gene content"),
        ...
        tablerTabItem(
          tabName = "genetic_evidence",
          tablerCard(
            title = "CNV Syndromes (DECIPHER and Clingen)",
  ...
  server = function(input, output, session) {
   ...
   choices_vector <- hpa %>% select(tissue) %>% distinct() %>% pull() %>% as.character()
    ...
    output$del_pubmed <- renderDataTable({
    ...
    output$abstract_del_pubmed <- renderDataTable({
      ...
      validate(
        need(input$del_pubmed_rows_selected != '', "Please, select an article from the datatable.")
      )
    ...
    output$dgenes <- renderDataTable({
      ...
      categorical_yes <- formatter("span", style = x ~ ifelse(x == 'Yes', 
                                                                   style(color = customRed, font.weight = "bold"), NA))
      
      continous_yes <-  formatter("span", style = x ~ ifelse(x %in% paste(seq(95,100)), 
                                                              style(color = customRed, font.weight = "bold"), NA))
      
      # data_input <- formattable(data_input, list(
      #   disease = categorical_yes,
      #   haplo = categorical_yes,
      #   triplo = categorical_yes,
      #   dev = categorical_yes,
      #   fda = categorical_yes,
      #   omim = categorical_yes,
      #   gwas = categorical_yes,
      #   
      #   
      #   pLI =continous_yes,
      #   rvis = continous_yes,
      ...
      #   hi = continous_yes,
      #   gdi = continous_yes,
      #   snipre = continous_yes,
      #   ncrvis = continous_yes,
      #   ncgerp = continous_yes
      #   
      #   
      #   
      #   
      ...
      # as.datatable(data_input, rownames = FALSE, filter = 'top', 
      #           # container = sketch,
      #           selection = 'single',
      ...
      #           )) %>%
        ...
        # formatStyle(c('pLI', 'rvis', 'hi', 'gdi', 'snipre', 'ncrvis', 'ncgerp'), color = styleInterval(95, c('weight', 'red')))
...
output$switch_tads <- renderUI({
  ...
  switchInput(
    ...
    output$hpo_assoc_genes <- renderDataTable({
      ...
      tmp_df2 <- tmp_df2 %>% mutate(hp = paste0("<a href='", paste0('https://hpo.jax.org/app/browse/term/', hp),"' target='_blank'>", hp,"</a>")) 
      test232134 <<- tmp_df2
      datatable(tmp_df2, escape = FALSE)
...
output$func_do  <- renderPlot({
  ...
  test3222 <<- running_do()
    ...
    running_de_novo <- reactive({
      ...
      start_coordinates <- as.numeric(coord_user()[1])
      end_coordinates <- as.numeric(coord_user()[2])
      chrom_coordinates <- as.numeric(coord_user()[3])
